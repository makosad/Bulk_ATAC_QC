...
${tool}/dependencies/2.subsamplingBamPE_byFract.sh $bam $dirO $tool # works

echo -e "\t\tStep 3:: summarize subsampling data per RL\n"
${tool}/dependencies/3.summarizeSubSampling_perRLs.sh $bam $dirO  # works

echo -e "\t\tStep 4:: plots per RL\n"
RL=$(basename ${bam/.bam/})
txt=$dirO"/5.Subsampling/${RL}/StatsAndPlots/${RL}_SubSamplingSummary.txt"
Rscript ${tool}/dependencies/4.plotsSubSamplingResults.R $txt 
